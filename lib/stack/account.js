define(['opjs/stack/util'], function (util) {
  ...
  function Account() {
    ...
    this._locations = {};
    // Map Subscription ID to Subscription
  ...
  Account.prototype._is = function (stateName) {
    return this._state === Account.States[stateName];
  };

  Account.prototype._setState = function (newState) {
    this._state = Account.States[newState];
  };

  Account.prototype.getPeerContactSession = function () {
    return this._peerContactSession;
  };
  ...
  Account.prototype.shutdown = function () {
    ...
    if (this._repository) {
      this._repository.cancel();
      this._repository.reset();
    }
    if (this._findersGetMonitor) {
      this._findersGetMonitor.cancel();
      this._findersGetMonitor.reset();
    }
    if (this._finderDNSLookup) {
      this._finderDNSLookup.cancel();
      this._finderDNSLookup.reset();
    }
    util.forEach(this._peerSubscriptions, function (sub) {
      sub.shutdown();
    });
    util.forEach(this._peerInfos, function (peerInfo) {
      if (peerInfo.peerFindMonitor) {
        peerInfo.cancel();
        peerInfo.reset();
      }
    });
